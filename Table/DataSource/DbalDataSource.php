...
class DbalDataSource implements DataSourceInterface
{
	...
	public function getData(ContainerInterface $container, array $columns, array $filters = null, Pagination $pagination = null, Order $sortable = null)
	{
		...
		if($pagination !== null)
		{
			...
			$this->query .= $paginatorStmt;
		}

		$stmt = $container->get('doctrine.orm.entity_manager')->getConnection()->prepare($this->query);
		$stmt->execute();
	...
	{
		$data = $this->query;
		$countQuery = $this->applyFilters($this->query, $filters);
		...
		try
		{
			...
			$countQuery = preg_replace("/\*/", $countStmt, $countQuery, 1);
